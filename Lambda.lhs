...
> bv t = case t of 
>        Lam x _ term -> Set.union (Set.singleton x) (bv term)
...
> fv (Lam (var) ty (lamterm)) = case lamterm of
>                            (App (lamterm1) (lamterm2)) -> Set.union (fv (Lam (var) ty lamterm1)) (fv (Lam (var) ty lamterm2))
>                            Var var1                    -> if var /= var1 then Set.singleton var1 else Set.empty
>                            (Lam (var1) ty (term1))      -> Set.intersection (fv (Lam (var) ty (term1))) (fv (Lam (var1) ty (term1)))
...
>                       (Lam (t1) ty (t2))               -> if x == t1 then Lam y ty (rename' t2 y x) else Lam t1 ty (rename' t2 y x)
...
>                       Lam binder ty bound -> if x /= binder then 
>                                               (if (Set.notMember binder (fv t1)) then Lam binder ty (subst (t1,x) bound) else subst (t1,x) newTerm) 
>                                           else term
...
> alphaEq t1@(Lam binder1 ty term1) t2@(Lam binder2 ty2 term2)    | binder1 == binder2 = term1 `alphaEq` term2
>                                                                 | otherwise = if (binder1 `Set.notMember` (vars term2)) 
>                                                                            then let nT2@(Lam nBind2 ty1 nT') = (rename t2 (binder1 ++ "'") binder2) 
>                                                                                     nT1@(Lam nBind1 ty2 nT)  = (rename t1 (binder1 ++ "'") binder1)
>                                                                                     in nT' `alphaEq` nT
>                                                                            else let nT2@(Lam nBind2 ty1 nT') = (rename t2 binder2 binder1)
>                                                                                  in term1 `alphaEq` nT'
> alphaEq _ _                                              = False   
